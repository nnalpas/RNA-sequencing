...
# Output value for MDS plot (dimension 1 and 4 in this case)
MDS <- plotMDS(x=Alv_norm, top=1000000, gene.selection="pairwise", xlab="Dimension 1", ylab="Dimension 4", dim.plot=c(1,4), cex=2.0)

# MDS values are then plotted with ggplot2
MDS_ggplot <- data.frame(target, MDS$x, MDS$y)
MDS_ggplot <- MDS_ggplot[,-1]
png(filename="MDS_Alv_mac(D1_vs_D4).png", width=1600, height=1000, units="px")
